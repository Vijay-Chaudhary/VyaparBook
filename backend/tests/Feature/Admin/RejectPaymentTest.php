...
it('refuses to reject an already-verified payment (422)', function () {
    ...
    expect(SubscriptionPayment::on('mysql_platform')->find($payment->id)->status)->toBe('verified');
    expect(Subscription::on('mysql_platform')->where('business_id', $business->id)->first()->status)->toBe('active');
