...
<style>
    ...
    .chart-tooltip {
        ...
        background: {{ config('charts.chrome.tooltip_bg') }}; color: {{ config('charts.chrome.tooltip_fg') }};
    ...
    @media (prefers-color-scheme: dark) {
        .chart-tooltip {
            background: {{ config('charts.chrome.tooltip_bg_dark') }};
            color: {{ config('charts.chrome.tooltip_fg_dark') }};
        }
    }
</style>
@endpush
...
(function () {
    ...
    document.addEventListener('mouseover', function (e) {
        ...
        var color = bar.getAttribute('data-color') || '{{ config('charts.series.sales') }}';
    ...
    <svg viewBox="0 -6 300 138" role="img" aria-label="{{ $title }}" class="w-full">
        ...
        @foreach ($tickData as $t)
            <line x1="{{ $plotLeft }}" y1="{{ $t['yPct'] }}" x2="{{ $plotRight }}" y2="{{ $t['yPct'] }}"
                  stroke="{{ config('charts.chrome.gridline') }}" stroke-width="0.3"></line>
            <text x="{{ $plotLeft - 2 }}" y="{{ $t['yPct'] + 2 }}" font-size="5"
                  text-anchor="end" fill="{{ config('charts.chrome.tick_label') }}">{{ $t['label'] }}</text>
        @endforeach
        {{-- zero baseline (bold) --}}
        <line x1="{{ $plotLeft }}" y1="{{ $baseline }}" x2="{{ $plotRight }}" y2="{{ $baseline }}"
              stroke="{{ config('charts.chrome.zero_line') }}" stroke-width="0.5"></line>
        ...
        @foreach ($groupData as $gi => $group)
            ...
            <text x="{{ $plotLeft + $gi * $slot + $slot / 2 }}" y="108" font-size="5"
                  text-anchor="middle" fill="{{ config('charts.chrome.group_label') }}">{{ $group['label'] }}</text>
        @endforeach
        ...
        @foreach ($legendData as $item)
            ...
            <text x="{{ $lx + 7 }}" y="124.5" font-size="5" fill="{{ config('charts.chrome.legend_label') }}">{{ $item['label'] }}</text>
