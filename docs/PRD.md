...
- **Append-only ledger** (sales, payments, returns as immutable entries; ~~corrections are new voiding entries~~ **corrections are voiding entries on the API and the order workflow, but the owner's console edits the row and soft-deletes it — the shop asked for its khata to say what happened rather than "sale ₹500, voided ₹500" for one mistyped sale**) → outstanding is always recomputable, and offline conflicts are near-eliminated. **A soft-deleted row still recomputes: it is excluded everywhere by one scope, and the deletion is undoable.**
